...
void Window::createTextures() {
    ...
    if (_useFXAA || Engine::instance().postProcessFunction()) {
        generateTexture(_frameBufferTextures.intermediate, TextureType::Color);
        glObjectLabel(
            GL_TEXTURE,
            _frameBufferTextures.intermediate,
            -1,
            std::format("Window{}_PostProcess_Intermediate", _id).c_str()
        );
    }
...
void Window::renderViewports(FrustumMode frustum, Eye eye) const {
    ...
    if (!isSplitScreen || frustum != FrustumMode::StereoLeft) {
        ...
        // Final copy to the output eye texture if the last pass ended in intermediate
        if (texColorOut[curOutIdx] != frameBufferTextureEye(eye)) {
            ...
            glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_2D, frameBufferTextureEye(eye));
